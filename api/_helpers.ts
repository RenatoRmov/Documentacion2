...
export function groupAlertsByVehicle(
  ...
  for (const v of fleet) {
    ...
    groups.push({
      ...
      conductor:      (v.nombre_conductor ?? v.nombreConductor ?? 'Sin nombre') as string,
      rutConductor:   (v.rut_conductor ?? v.rutConductor ?? '') as string,
      email:          (v.email as string) || null,
      celular:        (v.celular as string) || null,
      conductorToken: (v.conductor_token ?? v.conductorToken ?? null) as string | null,
      expired,
      upcoming,
      missing,
    });
